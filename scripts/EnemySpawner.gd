...
func spawn_enemy() -> void:
	...
	if current_wave == 1:
		...
		spawn = spawn_points[0]  # Spawn the boss only at the first spawn point
	...
	# Move to the next spawn point in the list, looping back to the start (except for wave 3)
	if current_wave < 3:  # Only increment spawn_index for the first two waves
		spawn_index += 1
